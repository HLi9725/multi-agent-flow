...
| **`/yy-flow run`** | **真实自动编排（2F-PROD 通用 Runner）**：读取任务并自动调度 Codex Builder → Antigravity Reviewer (JSON Schema) → Codex QA (源码不可变)，通过 EvidenceGate 验证后停在等待用户验收 | 运行 `python scripts/run_task.py start --task-id <id>` |
