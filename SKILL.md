...
| **`/yy-flow run`** | **真实自动编排（2F-PROD 通用 Runner）**：读取带逐项验收标准的任务，串行调度 Codex Builder → Antigravity Reviewer → Codex QA；Reviewer 必须审查影响面，QA 必须提交逐项覆盖、反向场景和命令证据，Runner 独立复跑全部受控命令，EvidenceGate 复核语义证据后停在等待用户验收 | 运行 `python scripts/run_task.py start --task-id <id>`；可重复传入 `--test-command` |
