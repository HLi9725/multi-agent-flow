---
name: yy-flow
description: 适用于 AI 多 Agent 与人类团队协同研发的多角色状态流转、质量审计、防错闭环与看板自动化工作流技能包。可通过 /yy-flow、/yy-flow start、/yy-flow status、/yy-flow kanban 等快捷指令或自然语言唤醒。
version: 1.0.0
---
...
| **`/yy-flow run`** | **真实自动编排（2F-PROD 通用 Runner）**：读取带逐项验收标准的任务，串行调度 Codex Builder → Antigravity Reviewer → Codex QA；Reviewer 必须审查影响面，Runner 针对每个候选 SHA 单次执行受控命令并把脱敏证据交给只读 QA，QA 必须提交逐项覆盖与反向场景，EvidenceGate 复核后停在等待用户验收 | 用户明确授权后运行 `python scripts/run_task.py start --task-id <id> --approve`；可重复传入 `--test-command`。Antigravity Prompt 通过 stdin `stream-json` 传输，最终 `structured_output` 与过程消息隔离；角色进度以 stderr `[YY-FLOW]` JSONL 事件输出。`resume` 默认继承 Checkpoint 配置；协议错误只重试原角色，达到上限先原子保存 `NEEDS_USER_INPUT`，工具缺失等基础设施故障停在原阶段，不得退回 Builder |
...
7. **架构师深度鉴定、能力拓展与 PM 终态验收**：
   ...
   - **显式输出 8 大专家子 Agent 列表与角色专属权限矩阵**：
     强制在回复中输出 Markdown 表格，清晰呈现所有子 Agent 标识、角色名称（前端专家【马前端】）、按角色边界装配的工具权限与合法状态流转区间。Reviewer/QA 默认只读，状态由 Production Runner 或主协调者核验结论后落库：
     | 子代理标识 | 角色名称 | 核心职责 | 工具权限 | 合法状态流转区间 |
     | :--- | :--- | :--- | :--- | :--- |
     | `@flow-pm` | 严经理 (项目经理) | 需求拆解、串行协调与终态验收 | 文档/看板写入 + run_command（不得修改业务代码） | 已完成->已验收 / 已完成->已退回 |
     ...
     | `@flow-reviewer` | 周审查 (代码审查专家) | 代码规范、安全扫描与质量门控 | 只读 + run_command（不得直接写看板） | 审查中->测试中 / 审查中->已退回 |
     | `@flow-qa` | 章测试 (测试工程师) | 集成测试、边界场景与质量准出 | 只读 + run_command（不得直接写看板） | 测试中->已完成 / 测试中->已退回 |
