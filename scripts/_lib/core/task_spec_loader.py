...
def load_task_execution_spec(
    ...
    spec = TaskExecutionSpec(
        ...
        cursor_model=(str(ov["cursor_model"]).strip() if ov.get("cursor_model") else None),
        cursor_api_key_env=str(ov.get("cursor_api_key_env", "CURSOR_API_KEY")).strip(),
        cursor_runtime=str(ov.get("cursor_runtime", "local")).strip(),
