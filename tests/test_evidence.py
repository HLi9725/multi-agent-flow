...
def dummy_metadata():
    return EvidenceMetadata(
        ...
        extra={"safe": "value", "capability_test": True}
    )
...
def expected_context(dummy_metadata):
    return {
        ...
        "baseline_commit": "abc",
        "result_commit": "def",
        "expected_capabilities": {"test": True}
    }
...
def test_store_no_overwrite(store, dummy_record):
    ...
    with pytest.raises(EvidenceSecurityError, match="forbidden"):
        store.append(dummy_record)
...
def test_gate_reject_fake_session(store, expected_context, dummy_metadata, tmp_path):
    ...
    store.append(rec)
    with pytest.raises(EvidenceGateError, match="Contains fake/test/mock/simulate identifier"):
...
# 8. Secret Masking + New Fields (access_token, env vars)
...
def test_canonical_json_and_immutability():
    # Deep Immutability (mapping, tuple, list, set)
    meta = EvidenceMetadata(
        ...
        transition_from="a", transition_to="b", created_at=0, extra={"nested": {"k": "v"}, "arr": [1, 2], "s": {3, 4}}
    )
    ...
    assert isinstance(meta.extra["s"], frozenset)
...
# 10. Concurrency collision override (True threading)
def test_store_concurrency_collision_and_override(store, dummy_record):
    ...
    for f in failures:
        ...
        assert "forbidden" in str(f) or "already exists" in str(f)
...
def test_gate_context_mismatch_none(store, dummy_metadata, expected_context, tmp_path):
    gate = EvidenceGate(store, str(tmp_path))
    # Context missing a key, evidence has it
    bad_context = dict(expected_context)
    del bad_context["host_session_id"]
    ...
    store.append(rec)
    with pytest.raises(EvidenceGateError, match="is missing in context"):
...
def test_gate_model_confirmation(store, dummy_metadata, expected_context, tmp_path):
    ...
    meta = EvidenceMetadata(
        **{**dummy_metadata.__dict__, "extra": {
            ...
            "user_source": "model", "capability_test": True
        }}
    )
    c_context = dict(expected_context)
    c_context.update({"confirmation_id": "c123", "confirmed_at": 1234567, "user_source": "model", "capability_test": True})
    ...
    store.append(rec)
    with pytest.raises(EvidenceGateError, match="must come from explicit user"):
        gate.validate_evidence("evt_conf", c_context)

# 14. Symlink / Junction test (No Skip)
def test_gate_junction_escape(store, dummy_metadata, expected_context, tmp_path):
    ...
    # Create link inside project root
    ...
    if os.name == 'nt':
        # Create Junction
        subprocess.run(f'cmd /c mklink /J "{link_path}" "{ext_dir}"', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    else:
